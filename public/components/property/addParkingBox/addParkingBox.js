        ...
        methods: {
            addParkingBoxValidate() {
                return vc.validate.validate({
                    addParkingBoxInfo: $that.addParkingBoxInfo
                }, {
            ...
            saveParkingBoxInfo: function() {
                if (!$that.addParkingBoxValidate()) {
                    vc.toast(vc.validate.errInfo);
                    return;
                }
                $that.addParkingBoxInfo.communityId = vc.getCurrentCommunity().communityId;
                //不提交数据将数据 回调给侦听处理
                if (vc.notNull($props.callBackListener)) {
                    vc.emit($props.callBackListener, $props.callBackFunction, $that.addParkingBoxInfo);
                    $('#addParkingBoxModel').modal('hide');
                    return;
                }
                vc.http.apiPost(
                    '/parkingBox.saveParkingBox',
                    JSON.stringify($that.addParkingBoxInfo), {
                        emulateJSON: true
                    },
                    function(json, res) {
