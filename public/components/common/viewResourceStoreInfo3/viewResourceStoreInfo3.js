    ...
    vc.extends({
        ...
        methods: {
            ...
            _changeTimesId: function (e, index) {
                ...
                let times = vc.component.viewResourceStoreInfo3.resourceStores[index].times;
                times.forEach((item) => {
                    if (item.timesId == timeId) {
                        // 存储价格对应库存，方便校验库存
                        vc.component.viewResourceStoreInfo3.resourceStores[index].selectedStock = item.stock;
                    }
                })
            },
